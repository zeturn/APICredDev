...
async def get_user_usage_summary(db: AsyncSession, user_id: str) -> dict:
    model_rows = await db.execute(select(Model.id, Model.name))
    model_name_map = {row.id: row.name for row in model_rows.all()}
...
async def get_admin_usage_summary(db: AsyncSession) -> dict:
    ...
    provider_rows = await db.execute(select(ProviderKey.id, ProviderKey.provider, ProviderKey.key_name))
    provider_map = {
